...
-- ORBITRIP DATABASE SCHEMA (v2.4 - Author Tours & Build Fixes)
...
CREATE TABLE IF NOT EXISTS public.tours (
  ...
  author_id text -- Link to drivers table
);
...
-- MIGRATION: Add author_id to tours if missing
DO $$ 
BEGIN 
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='tours' AND column_name='author_id') THEN 
    ALTER TABLE public.tours ADD COLUMN author_id text; 
  END IF; 
END $$;
