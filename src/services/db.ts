...
const INITIAL_MOCK_TOURS: Tour[] = [
    ...
    },
    {
        ...
        itineraryRu: ["Посещение Batumi: Start.","Посещение Ботанический Сад: Garden.","Посещение Petra: Fortress.","Посещение Batumi (Finish): Finish."],
...
export const db = {
    ...
    backup: {
        generateDump: async (): Promise<string> => {
            ...
            sqlDump += `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='tours' AND column_name='author_id') THEN ALTER TABLE public.tours ADD COLUMN author_id text; END IF; END $$;\n\n`;
    ...
    smsLogs: {
        ...
        log: async (log: SmsLog) => {
            if (!isSupabaseConfigured) return;
            await supabase.from('sms_logs').upsert([log]);
        }
    },
    promoCodes: {
