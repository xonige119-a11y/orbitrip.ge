...
const App = () => {
    ...
    useEffect(() => {
        ...
        console.clear();
        console.log("APP INIT: v8.0.1 Loaded Successfully");
    ...
    useEffect(() => {
        const handlePopState = (event: PopStateEvent) => {
            if (event.state && event.state.view) {
                ...
                } else if (currentView.startsWith('LEGAL') || currentView === 'SITEMAP') {
                    setCurrentView('HOME');
                } else if (currentView === 'DRIVER_REGISTRATION') {
                    setCurrentView('HOME');
                }
            }
        };
    ...
    const handleBookingSubmit = async (bookingData: any) => {
        try {
            ...
            // Identify the actual driver object securely, either from state or by looking up ID
            const assignedDriver = selectedDriverForBooking || drivers.find(d => d.id === bookingData.driverId);

            // 1. Send Admin Notification (SMS)
            ...
            // 2. Send Customer Email Confirmation (Now includes driver info if available)
            emailService.sendBookingConfirmation(newBooking, selectedTour, language, assignedDriver || null).catch(() => {});

            // 3. Send Driver Notifications (If driver is assigned)
            if (assignedDriver) {
    ...
    const baseSeoTitle = useMemo(() => {
        ...
        switch(currentView) {
            ...
            case 'SITEMAP': return isEn ? "All Routes" : "Все Маршруты";
            case 'DRIVER_REGISTRATION': return isEn ? "Become a Driver" : "Стать Водителем";
        ...
        <ErrorBoundary language={language}>
            ...
            <Header 
                ...
                onToolSelect={(tool) => {
                    ...
                    else if (tool === 'SITEMAP') navigateTo('SITEMAP', '/routes');
                }} 
                ...
                {(() => {
                    switch (currentView) {
                        ...
                        case 'SITEMAP':
                             return (
                                 <div className="pt-24 bg-white">
                                     <SitemapView language={language} onBack={handleReset} onLinkClick={(f, t) => handleSearch({ stops: [f, t], date: '', totalDistance: 0 }, false)} />
                                 </div>
                             );
            ...
            {['HOME', 'TOURS', 'BLOG', 'SEARCH_RESULTS', 'DRIVER_PROFILE', 'LEGAL_TERMS', 'LEGAL_PRIVACY', 'SITEMAP'].includes(currentView) && (
