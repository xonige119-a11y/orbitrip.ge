...
const DriverProfile: React.FC<DriverProfileProps> = ({ driver, language, onBack, onBook, price, date }) => {
  ...
  const displayedReviews = reviewsList.slice(0, visibleReviewsCount);
              ...
              <div className="lg:col-span-2 space-y-6 md:space-y-8">
                  ...
                  <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100">
                      <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center justify-between">
                          ...
                          <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-xs font-bold">{reviewsList.length}</span>
                      </h3>
                      ...
                      {reviewsList.length > visibleReviewsCount && (
                          <div className="mt-8 text-center">
                              <button 
                                  ...
                                  className="bg-white border border-gray-300 text-gray-700 font-bold py-3 px-8 rounded-xl hover:bg-gray-50 hover:border-gray-400 transition-all shadow-sm active:scale-95 text-sm"
                              >
                                  {isEn ? `Show More (${reviewsList.length - visibleReviewsCount})` : `Показать еще (${reviewsList.length - visibleReviewsCount})`}
                              </button>
                          </div>
                      )}
                  </div>
