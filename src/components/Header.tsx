...
interface HeaderProps {
  ...
  isLoggedIn?: boolean; // Added optional property
}
    ...
    >
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <div className="flex items-center space-x-4">
                ...
                <button 
                    ...
                    className="text-xs font-bold text-white hover:bg-white hover:text-slate-900 transition uppercase tracking-wide border border-white/40 bg-white/5 backdrop-blur-md px-4 py-2 rounded-lg hover:shadow-lg flex items-center gap-2"
                >
                    {isLoggedIn ? (isEn ? 'Dashboard' : 'Кабинет') : (isEn ? 'Partner Login' : 'Партнерам')}
                </button>
            </div>
          </div>
      ...
      <div className={`fixed inset-0 z-40 bg-slate-900 transition-transform duration-300 md:hidden flex flex-col pt-24 px-6 space-y-6 h-screen overflow-y-auto ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex justify-between items-center mt-4">
           ...
           <button 
                onClick={() => handleNavClick(isLoggedIn ? 'DRIVER_DASHBOARD' : 'ADMIN_LOGIN')}
                className="w-full py-4 rounded-xl border border-white/20 text-white font-bold hover:bg-white/10 transition mt-auto mb-8 flex items-center justify-center gap-2"
           >
                {isLoggedIn ? (isEn ? 'Dashboard' : 'Кабинет') : (isEn ? 'Partner Login' : 'Вход для Партнеров')}
           </button>
      </div>
    </header>
    </>
