...
const TripSearchBox: React.FC<TripSearchBoxProps> = ({ language, onSearch, initialStops, initialDate, maintenanceMode = false }) => {
  ...
  const handleManualSearch = (e: React.FormEvent) => {
    ...
    if (data) {
        ...
        // --- GOOGLE ANALYTICS: route_calculated ---
        console.log("[GA4] Attempting route_calculated event");
        if ((window as any).gtag) {
            (window as any).gtag('event', 'route_calculated', {
                origin: data.stops[0],
                destination: data.stops[data.stops.length - 1],
                total_distance: data.totalDistance,
                date: data.date,
                debug_mode: true 
            });
            console.log("[GA4] Event sent: route_calculated");
        } else {
