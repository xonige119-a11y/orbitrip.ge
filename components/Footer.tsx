...
const Footer: React.FC<FooterProps> = ({ language, settings, onNavigate }) => {
  ...
  const time = new Date().toLocaleTimeString(); // LIVE TIME CHECK
  ...
  return (
    <footer className="bg-[#0f172a] text-slate-400 font-sans pt-10 md:pt-16 pb-8 border-t border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 mb-12 md:mb-16">
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm">
                ...
                <li>
                    <a href="https://admin.orbitrip.ge" target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:text-emerald-300 font-bold transition-colors flex items-center gap-2">
                        <span>🚖</span> {isEn ? "Become a Driver" : "Стать Водителем"}
                    </a>
                </li>
            </ul>
          </div>
        ...
        {/* BOTTOM BAR */}
        <div className="pt-8 border-t border-slate-800 flex flex-col items-center justify-center gap-4 text-xs font-medium text-center">
            <p>© {year} OrbiTrip Georgia. {isEn ? "All rights reserved." : "Все права защищены."}</p>
            <div className="flex gap-6 items-center">
                <span className="text-slate-500 font-mono">Last Built: {time}</span>
                <span className="text-emerald-400 font-bold bg-emerald-900/30 px-3 py-1 rounded-full border border-emerald-500/50 animate-pulse">v7.0 (USER ONLY)</span>
            </div>
        </div>
